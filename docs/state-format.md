...
`context.json` может создаваться `tools/kotoba_session.py` как компактный производный кэш для нового чата. Не редактировать его вместо канонических файлов и не использовать как единственную резервную копию.
